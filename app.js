...
async function resolveStreamUrl(url) {
  try {
    ...
    if (encoded) {
      ...
      if (decoded.startsWith('http')) {
        /* Upgrade plain http:// decoded URLs through the HTTPS proxy to
           avoid mixed-content blocks on the live HTTPS domain.          */
        if (decoded.startsWith('http://')) {
  ...
  /* Also upgrade any bare http:// stream URLs passed directly */
